...
from django.shortcuts import render
import logging
from .forms import MisDatosForm, UsuarioForm
...
def boleta(request, nro_boleta):
    try:
        ...
        mensaje = "No se encontró la boleta solicitada."
...
def ficha(request, producto_id):
    ...
    context = obtener_info_producto_btn(producto_id)
...
def man_productos(request, accion, id):
    ...
    if request.method == 'GET':
        ...
        if accion == 'crear':
            ...
            return redirect(man_productos, 'crear', '0')
...
def ropa(request):
    ...
    return render(request, 'core/ropa.html', {'products': products})
...
def usuarios(request,accion,id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(usuarios, 'actualizar', user.id)
        else:
            messages.error(request, f'No se pudo {accion} el Usuario, pues el formulario no pasó las validaciones básicas')
...
def ventas(request):
    ...
    try:
        # Obtener la página correspondiente del paginador
...
def obtener_info_producto(producto_id):
    ...
    con_oferta = f'<span class="text-success fw-bolder"> EN OFERTA {producto.descuento_oferta}% DE DESCUENTO </span>'
    sin_oferta = f'<span class="text-info fw-bolder"> DISPONIBLE EN BODEGA </span>'
    agotado = f'<span class="text-danger fw-bolder"> AGOTADO </span>'
    ...
    en_stock = f' {formatear_numero(stock)} {"unidad" if stock == 1 else "unidades"}'
...
def obtener_info_producto_btn(producto_id):
